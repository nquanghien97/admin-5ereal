...
interface TienIchProps {
  listImagesTienIch: File[]
  setListImagesTienIch: React.Dispatch<React.SetStateAction<File[]>>
}

function TienIch(props: TienIchProps) {
  const { listImagesTienIch, setListImagesTienIch } = props
  ...
  return (
    <>
      <div className="max-w-7xl p-4 m-auto border border-dashed mb-8">
        <div>
          <h2 className="text-3xl md:text-5xl font-[800] text-[#0F3E5A] text-center mb-4">TIỆN ÍCH</h2>
          <Form.Item
            label="Mô tả tiện ích"
            className="!px-4"
            name="description_tien_ich"
            rules={[
              {
                required: true,
          ...
          <div className="flex justify-end mt-4 gap-4">
            ...
            {listImages && (
              <Button
                type="primary"
                onClick={() => {
                  setListImagesTienIch(listImages)
                  setIsOpenModalAddImages(false)
                }}
              >
                Tải lên
