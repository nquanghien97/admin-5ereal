...
function AddProjects(props: AddProjectsProps) {
  ...
  const onFinish = async (data: FormValues) => {
    ...
    try {
      ...
      notification.success('Thêm Dự án thành công')
      onClose();
      ...
      if (err instanceof Error) {
        if (err.message === "Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại.") {
          ...
          notification.error('Thêm Dự án thất bại')
        }
      }
    } finally {
      setLoading(false);
  ...
  return (
    <Modal
      ...
      footer={false}
    >
      <div className="w-full text-center p-3 h-[60px] leading-[36px] bg-[#84571B] rounded-t-lg uppercase font-bold">Thêm Dự án</div>
      <div className="p-4">
